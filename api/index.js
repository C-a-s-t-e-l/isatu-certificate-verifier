...
app.get("/api/certificates", async (req, res) => {
  ...
  try {
    let query = supabase
      .from("certificates")
      .select("control_number, recipient_name, event_name, date_given", {
        count: "exact",
      });

    if (searchQuery) {
      query = query.or(
        `recipient_name.ilike.%${searchQuery}%,event_name.ilike.%${searchQuery}%,control_number.ilike.%${searchQuery}%`
      );
    }
...
app.get("/api/analytics/summary", async (req, res) => {
  try {
    ...
    const [
      ...
      supabase.rpc("count_total_recipients"),
    ]);
    ...
    const errors = [
      ...
      monthlyRes.error,
      topEventsRes.error,
      topEncodersRes.error,
      topRecipientsRes.error,
      totalRecipientsRes.error,
    ].filter(Boolean);
    if (errors.length > 0) {
    ...
    const busiestMonth = monthlyRes.data.length
      ? monthlyRes.data.reduce((prev, current) =>
    ...
    res.json({
      ...
      totalEvents,
      totalRecipients: totalRecipientsRes.data,
      busiestMonth,
      byType: byTypeRes.data,
      trends: trendsRes.data,
      topEvents: topEventsRes.data.slice(0, 10),
      topEncoders: topEncodersRes.data.slice(0, 10),
      topRecipients: topRecipientsRes.data.slice(0, 10),
    });
  } catch (err) {
    console.error(err);
...
app.post("/api/export/:format", async (req, res) => {
  ...
  try {
    ...
    if (format === "xlsx") {
      ...
      doc
        .fontSize(16)
        .text("ISATU Student Republic - Certificate Export", {
          align: "center",
        });
      doc
        .fontSize(10)
      ...
      headers.forEach((header) => {
        doc
          .fontSize(10)
          .text(header, 30 + i, tableTop, {
            width: colWidths[headers.indexOf(header)],
            align: "left",
          });
      ...
      data.forEach((cert) => {
        ...
        doc.text(new Date(cert.date_given).toLocaleDateString(), 500, rowY, {
          width: 60,
        });
...
app.post("/api/generate", async (req, res) => {
  ...
  if (!certData.recipient_name || !certData.event_name) {
    return res
      .status(400)
      .json({ message: "Recipient and Event Name are required." });
  }
  try {
    let controlNumber = certData.control_number;
    if (!controlNumber || controlNumber.toUpperCase() === "N/A") {
      const typePrefix = certData.certificate_type.startsWith("App")
        ? "CA"
        : certData.certificate_type.startsWith("Part")
        ? "CP"
        : "CR";
      ...
      if (lastCert && lastCert.length > 0) {
        nextNum = parseInt(lastCert[0].control_number.split("-").pop(), 10) + 1;
      }

      controlNumber = `ISATU-SR-${typePrefix}-25-${String(nextNum).padStart(
        5,
        "0"
      )}`;
    }
    const newCertificateData = {
      ...
      timestamp:
        certData.timestamp && certData.timestamp !== "N/A"
          ? new Date(certData.timestamp).toISOString()
    ...
    if (insertError) {
      ...
      if (insertError.code === "23505") {
        return res
          .status(409)
          .json({
            message: `Error: Control Number ${controlNumber} already exists.`,
          });
      }
      throw insertError;
    }
    ...
    res
      .status(500)
      .json({ message: "Server error during certificate processing." });
  }
});
...
app.post("/api/generate-bulk", async (req, res) => {
  ...
  if (error && error.code === "23505") {
    ...
    return res
      .status(207)
      .json({ message: "Partial success. Some records failed.", results });
  }
  ...
  const successResults = certificates.map((cert) => ({
    control_number: cert.control_number,
    status: "success",
  }));
  res
    .status(201)
    .json({
      message: "All certificates added successfully.",
