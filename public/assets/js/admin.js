...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const initializeAdminPage = async () => {
    ...
    const SHADCN_COLORS = {
        ...
        tooltip: 'hsl(222.2, 47.4%, 11.2%)',
        barPalette: ['#2c3e50', '#7f8c8d', '#95a5a6', '#bdc3c7', '#ecf0f1']
    };

    const getStyledChartOptions = (overrides = {}) => {
        const baseOptions = {
            responsive: true,
            ...
            plugins: {
                ...
                tooltip: {
                    backgroundColor: SHADCN_COLORS.tooltip,
        ...
        if (overrides.plugins) {
            mergedOptions.plugins = { ...baseOptions.plugins, ...overrides.plugins };
        }
    ...
    const loadAnalyticsData = async (period = 'daily') => {
        try {
            ...
            renderChart(document.getElementById('type-chart').getContext('2d'), 'doughnut', {
                ...
                datasets: [{ 
                    ...
                    backgroundColor: [SHADCN_COLORS.primary, SHADCN_COLORS.accent, SHADCN_COLORS.muted],
                    borderWidth: 0
                }]
            }, getStyledChartOptions({ 
            ...
            const reversedTopEvents = data.topEvents.slice();
            renderChart(document.getElementById('events-chart').getContext('2d'), 'bar', {
                labels: reversedTopEvents.map(d => d.event),
                datasets: [{ 
                    label: 'Attendees', 
                    data: reversedTopEvents.map(d => d.count),
                    backgroundColor: SHADCN_COLORS.barPalette,
                    maxBarThickness: 40
                }]
            }, getStyledChartOptions({ 
                indexAxis: 'y', 
            ...
            const reversedTopEncoders = data.topEncoders.slice();
            renderChart(document.getElementById('encoders-chart').getContext('2d'), 'bar', {
                labels: reversedTopEncoders.map(d => d.encoder),
                datasets: [{ 
                    label: 'Certs Encoded', 
                    data: reversedTopEncoders.map(d => d.count),
                    backgroundColor: ['#c0392b', '#d35400', '#e67e22', '#f39c12', '#f1c40f'],
                    maxBarThickness: 40
                }]
            }, getStyledChartOptions({ 
                indexAxis: 'y', 
    ...
    const handleExport = async (format) => {
        let payload = null;
        let exportUrl = `${API_BASE_URL}/export/${format}`;
        const useSelection = selectedControlNumbers.size > 0;

        if (useSelection) {
            payload = { ids: Array.from(selectedControlNumbers) };
        }
